...
## 100-topic release (September 26, 2026)

Run `build_content_sprint_100.py` after legacy page and case generators. It maintains 16 new guides, 21 rewrites and 63 expansions listed in `content_sprint_100.tsv`, normalizes the discovery CTA to Book a Call, and keeps rewritten hypothetical case cards consistent. Cost segregation remains separately priced. Then rebuild blog and comparison indexes, rerun the sprint to restore collection links, rebuild the outcomes report and HTML sitemap, preserve existing XML lastmod dates and update only substantive release URLs, and rebuild discovery files. Never publish the older case generators without applying these editorial overrides.
