#!/usr/bin/env python3
"""Third-party validation: published press mentions, linked and machine-readable.

Two channels, because answer engines use both.

Visible: a full list of the features as real outbound anchors on the pages a
"is this firm real" query lands on. A logo strip naming outlets without linking
to them proves nothing a crawler can follow.

Structured: the same URLs added to the Organization node as ``subjectOf``
...
# Where a legitimacy check actually lands. All headlines inline run several thousand
