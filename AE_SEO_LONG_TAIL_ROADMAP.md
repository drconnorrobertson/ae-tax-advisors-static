...
- Publication and indexing check: Commit `edf875b6` was pushed to `main`. Full gates passed: 1,301 canonical indexable pages match 1,301 sitemap URLs; content-quality errors/warnings 0, broken internal links 0, date conflicts 0, invalid JSON-LD 0, and scaled-content errors/warnings 0 for the one new URL. The full validator retains unrelated legacy/staging notices outside affected pages. The live new canonical returned HTTP 200 with the direct-answer H1, self-canonical, three button links to `/discovery/` (navigation plus two standalone content CTAs), three distinct IRS primary-source URLs, and a September 24 sitemap entry; the LP guide and blog index link to it live. IndexNow accepted all three changed canonical URLs, HTTP 200 (3/3). Authenticated Google Search Console currently says the new URL is unknown to Google, while its post-deployment live test says “URL is available to Google,” “Page can be indexed,” and detects one valid breadcrumb item. The submitted sitemap reports Success but 1,296 discovered pages, behind the current 1,301-page XML. Manual indexing request and current sitemap resubmission await action-time approval. No public Google Indexing API was used; counts remain conservative until this gate is resolved.
