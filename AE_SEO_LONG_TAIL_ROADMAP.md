...
### September 24, 2026 — Pending validation/publication: Form 3115 on an amended return
...
- Validation and publication: 1,300 canonical indexable URLs equal 1,300 sitemap URLs; content-quality errors/warnings 0, broken internal links 0, date conflicts 0, invalid JSON-LD 0, scaled-content errors/warnings 0 for zero new pages; `git diff --check` clean. The four visible FAQ answers match FAQPage JSON-LD, the title is 55 characters, the description 149 characters, and both exact booking CTAs stand alone. The full validator retains only unchanged unrelated legacy/staging notices. Commit `8d27f557` was pushed to main. The [live canonical](https://www.aetaxadvisors.com/amended-return-vs-form-3115/) returned HTTP 200 with the revised H1, self-canonical, two standalone CTAs, and September 24 sitemap lastmod. IndexNow accepted the changed canonical, HTTP 200 (1/1). Authenticated Search Console says the established URL is indexed; a September 24 post-deployment live test says the revision is available to Google, can be indexed, and has one valid breadcrumb. The existing sitemap shows Success but 1,296 discovered pages, lagging the 1,300 live XML count. Manual recrawl and confirmatory sitemap resubmission await action-time approval. Campaign count stays **67/100** until the final gate; the separate new-URL count stays **5/200**. No public Google Indexing API was used.
