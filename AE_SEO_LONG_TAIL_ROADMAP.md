...
- Pre-publication validation: 1,299 canonical indexable URLs match 1,299 sitemap URLs; content-quality errors/warnings 0; broken internal targets 0; date conflicts 0; invalid JSON-LD 0; scaled-content errors/warnings 0 for zero new URLs; `git diff --check` clean. `seo_validate.py` retains unchanged legacy/staging notices outside the affected pages. Publication, live checks, IndexNow, and authenticated Search Console inspection remain required before this target can be accepted. Campaign count remains **67/100** pending those gates.
