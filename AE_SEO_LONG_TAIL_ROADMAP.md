...
### September 24, 2026 — Pending publication and indexing: basis-suspended partnership losses on sale
...
- Status: Commit `a7d06ba9` was pushed to main. Full gates passed: 1,300 canonical indexable URLs equal 1,300 sitemap URLs; content-quality errors/warnings 0, broken internal links 0, date conflicts 0, invalid JSON-LD 0, scaled-content errors/warnings 0 for zero new pages, and `git diff --check` clean. The broader validator retains only unrelated legacy/staging notices. The live canonical returned HTTP 200 with the revised H1, self-canonical, two standalone exact booking CTAs, and September 24 sitemap lastmod. IndexNow accepted the changed canonical, HTTP 200 (1/1). Authenticated Search Console says this established URL is indexed; a September 24 post-deployment live test says the revision is available to Google, can be indexed, and has one valid breadcrumb item. The submitted sitemap shows Success but 1,296 discovered pages, behind the current 1,300-page XML. Manual recrawl and confirmatory sitemap resubmission await action-time approval. Campaign count stays **67/100** until that final gate; the new-URL count stays **5/200**. No public Google Indexing API was used.
