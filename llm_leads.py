...
TOPICS: list[tuple[str, list[str], str]] = [
    ...
    # The "digital-assets" topic was removed when crypto was taken out of scope.
    # Its keyword list was deleted but the entry was left behind as a 2-tuple,
    # which made TOPIC_BY_ID raise IndexError and broke `import llm_leads` (and
    # so llm_build) outright. Removed rather than repaired: crypto is out of
    # scope, so the topic should not come back.
    (
        "rental",
...
DISABLED = (
    "llm_leads is disabled. The per-page definition paragraphs it writes are no\n"
    "longer wanted in the HTML: that content is served from /llms.txt, /llms.md\n"
    "and /.well-known/llms.txt, generated by llm_txt.py.\n\n"
    "Running this would reinject a boxed paragraph into ~1,257 pages. If you\n"
    "genuinely want that back, pass --force, and re-add it to llm_build.STEPS.\n"
    "To remove leads again afterwards, run strip_llm_leads.py.\n"
)
